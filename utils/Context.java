    ...
    public void run() {
        ...
        while (true) {
            ...
            if (!wasError && this.isTrackable(operation)) {
                // track vcs operations
                switch (operation.type) {
                    case TOUCH:
                            vcs.setTrace("Created file " + operation.getOperationArgs().get(1));
                        break;
                    case WRITETOFILE:
                        vcs.setTrace("Added " + "\"" +  operation.getOperationArgs().get(0)
                                + "\" to file " + operation.getOperationArgs().get(1));
                        break;
                    case REMOVE:
                        vcs.setTrace("Removed " + operation.getOperationArgs().get(1));
                    case MAKEDIR:
                        vcs.setTrace("Created directory " + operation.getOperationArgs().get(0));
                        break;
                    case CHANGEDIR:
                        vcs.setTrace("Changed directory to " + operation.getOperationArgs().get(0));
                        break;
                    default:
                        break;
                }
